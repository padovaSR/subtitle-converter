...
class MyFrame(ConverterFrame):
    def __init__(self, *args, **kwds):
        ...
        *.txt)|*.zip;*.srt;*.txt|MicroDVD (*.sub)|*.sub|Text File\
        ...
        with shelve.open(
            filePath('resources', 'var', 'dialog_settings.db'), flag='writeback'
        ) as s:
            ex = s['key4']
    ...
    def handleFile(self, filepaths):
        ...
        tmp_path = [
            os.path.join(self.location, os.path.basename(item)) for item in inpaths
        ]  # sef.locatin is tmp/
            ...
            def ansiAction(inpath):
                ...
                if text:
                    msginfo = wx.MessageDialog(
                        self,
                        f'Tekst je konvertovan u enkoding: {self.newEnc.upper()}.',
                        'SubtitleConverter',
    ...
    def toCyrillic(self, event):
        ...
        if len(self.multiFile) >= 1:
            ...
            if os.path.exists(utf_path):
                nnm = (
                    fproc.nameCheck(os.path.basename(utf_path), self.real_dir, suffix)
                    + 1
                )
    ...
    def toUTF(self, event):
        ...
        if len(self.multiFile) >= 1:
            ...
            if text:
                ...
                msginfo = wx.MessageDialog(
                    self,
                    f'Tekst je konvertovan u enkoding: {code}.',
                    'SubtitleConverter',
                    wx.OK | wx.ICON_INFORMATION,
                )
                msginfo.ShowModal()
    ...
    def exportZIPmultiple(self):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if list(self.previous_action.keys())[-1] == 'toCYR_multiple':
                ...
                try:
                    izbor = [
                        os.path.join("tmp", x) if not x.startswith("tmp") else x
    ...
    def onMergeLines(self, event):
        ...
        if len(subs_a) > 0:
            ...
            try:
                ...
                logger.debug(
                    "Merger: Spojenih linija ukupno: {}, ili {} %".format(a1 - b1, prf)
                )
                sDlg = wx.MessageDialog(
                    self,
                    "Merged file:\n\nSpojenih linija ukupno: {0}, ili {1} %".format(
                        a1 - b1, prf
                    ),
                    'SubtitleConverter',
                    wx.OK | wx.ICON_INFORMATION,
    ...
    def onSave(self, event):
        ...
        if tpath and enc:
            ...
            outpath = fproc.nameDialog(
                fname, nsuffix, self.real_dir
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if list(self.previous_action.keys())[-1] == "toCYR":
                    ...
                    if dlg.ShowModal() == wx.ID_OK:
                        ...
                        if len(files) == 1:
                            try:
                                ...
                                logger.debug(
                                    "Export ZIP, IOError({0}{1}):".format(fpath, e)
                                )
                            except:
                                exc_type, exc_value, exc_traceback = sys.exc_info()
                                lines = traceback.format_exception(
                    ...
                    if previous_action == 'toCyrSRTutf8':
                        ...
                        info2 = (
                            os.path.basename(self.real_path)[:-3] + presuffix + suffix
                        )
                        info1 = None
                        lat_file = None
                    else:
                ...
                try:
                    with zipfile.ZipFile(name, 'w') as fzip:
                        for i, x in zip(
                            [info1, info2, lat_file], [zdata, tzdata, ldata]
                        ):
                            if i == None:
                                continue
                            if len(x) == 0:
    ...
    def onFixSubs(self, event):
        ...
        if len(subs) == 0:
            ...
            try:
                ...
                logger.debug(
                    "FixSubtitle, I/O error({0}): {1}".format(e.errno, e.strerror)
                )
            except Exception as e:
    ...
    def onUndo(self, event):
        ...
        if self.undo_text:
            ...
            self.undo_text = self.undo_text[:-1]
        if len(self.redo_text) >= 40:
            self.redo_text = self.redo_text[1:]
...
class MyApp(wx.App):
    ...
    def m_files(self):
        ...
        if not os.path.isfile(d_file):
            missing_f.append(d_file)

        if missing_f:
            error_text = "File Not Found\n\n{}\nPlease check files missing!".format(
                "".join([x + '\n' for x in missing_f])
            )
            ErrorDlg = wx.MessageDialog(
